...
public final class ReconnectingThriftClient {
  ...
  /**
   * List of causes which suggest a restart might fix things (defined as constants in {@link
   * org.apache.thrift.transport.TTransportException}).
   */
  private static final Set<Integer> RESTARTABLE_CAUSES =
      Sets.newHashSet(
  ...
  public static class Options {
    ...
    /**
     * @param numRetries the maximum number of times to try reconnecting before giving up and
  ...
  /**
   * Reflectively wraps a thrift client so that when a call fails due to a networking error, a
   ...
   *     {@link #wrap(org.apache.thrift.TServiceClient, ReconnectingThriftClient.Options)}
   * @param options options that control behavior of the reconnecting client
   * @param <T>
   * @param <C>
   * @return
   */
  public static <T extends TServiceClient, C> C wrap(
      T baseClient, Class<C> clientInterface, Options options) {
    Object proxyObject =
        Proxy.newProxyInstance(
            clientInterface.getClassLoader(),
            new Class<?>[] {clientInterface},
            new ReconnectingClientProxy<T>(
  ...
  /**
   * Helper proxy class. Attempts to call method on proxy object wrapped in try/catch. If it fails,
   * it attempts a reconnect and tries the method again.
   *
   * @param <T>
   */
  private static class ReconnectingClientProxy<T extends TServiceClient>
        ...
        if (e.getTargetException() instanceof TTransportException) {
          ...
          if (RESTARTABLE_CAUSES.contains(cause.getType())) {
            reconnectOrThrowException(
                baseClient.getInputProtocol().getTransport(), maxRetries, timeBetweenRetries);
            return method.invoke(baseClient, args);
          }
        }
        throw e;
    ...
    private static void reconnectOrThrowException(
        TTransport transport, int maxRetries, long timeBetweenRetries) throws TTransportException {
      int errors = 0;
      transport.close();
