...
def _get_basic_aggregations_ctf_reals_mongodb_aggregation(
    client: MongoClient,
    sensor_ids: List[int],
    physical_quantity: str,
    from_timestamp: int,
    to_timestamp: int,
) -> List[PhysicalQuantityDataPoint]:
    ...
    cursor = collection.aggregate(
        [
            ...
            {"$unwind": {"path": "$measurements"}},
            {"$match": {"measurements.name": physical_quantity}},
            {"$sort": {"timestamp": 1}},
            {
                "$group": {
                    "_id": "$sensorId",
                    "value": {"$last": "$measurements.value"},
